...
public class BasketballHoopControl : MonoBehaviour
{
    ...
    private IEnumerator RotateAroundCoroutine(Vector3 rotationalPivot)
    {
        if (!isRotationAroundCoroutine)
        {
            ...
            yield return StartCoroutine(
                backBoardFadingControl.FadeCoroutine(false, false));
            ...
            yield return StartCoroutine(
                backBoardFadingControl.FadeCoroutine(true, true));
    ...
    {   
        ...
        else
        {
            ...
            {
                positionZCurrent = UnityEngine.Random.Range(
                    positionZStepMin, positionZStepMax);
    ...
    private void RotateAroundBase(Vector3 rotationPivot)
    {
        if (isFirstRotation)
        {
            ...
            vectorDiffHoopAndRotationalPivotInit =
                hoop.transform.position - rotationPivot;
        ...
        angleHoopAndRotationalPivot =
            angleHoopAndRotationalPivotAbs * (Vector3.Cross(
                    vectorDiffHoopAndRotationalPivotInit,
                    vectorDiffHoopAndRotationalPivotCurrent).y
                > 0f ? 1f : -1f);
        ...
        rotationalAngleRandom = Mathf.Clamp(
            angleHoopAndRotationalPivot + rotationalAngleRandom,
            rotationalAngleMin, rotationalAngleMax)
                - angleHoopAndRotationalPivot;

        hoop.transform.RotateAround(
            rotationPivot, Vector3.up, rotationalAngleRandom);
    }
